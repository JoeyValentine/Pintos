...
void 
parse_filename(const char *file_name, char *argv[], int *argc)
{
...
void 
construct_ESP(char *argv[], const int argc, void **esp)
{
  char *first_argv_addr;
  void (*ret_addr)(void) = 0;
  char *argv_addr[MAX_ARGV_LEN];
  int i, align_num = 0;
  char null_str = 0;
  ...
  // suppose that argc < MAX_ARGV_LEN
  argv_addr[argc] = 0;

  align_num = 4 - align_num % 4;
  *esp -= align_num;
  memset(*esp, null_str, align_num);
  ...
  first_argv_addr = *esp;
  *esp -= sizeof(char**);
  memcpy(*esp, &first_argv_addr, sizeof(char**));
  ...
  *esp -= sizeof(void(*));
  ...
  //hex_dump((int)*esp, *esp,last_pos-*esp,true);
...
{
  char *real_file_name, *save_ptr;
// we assume that file name is not longer than 16
  char temp[64] = {0, }; 
  ...
  real_file_name = strtok_r(temp, " ", &save_ptr);
  ...
  if (tid == TID_ERROR)
  ...
  else
    {
      ...
      if( parent->did_child_load_exec == false )
	tid = TID_ERROR;
    }
...
start_process (void *file_name_)
{
  ...
  t->parent->did_child_load_exec = success;
...
process_wait (tid_t child_tid UNUSED) 
{
  ...
  child_exit_status = parent->child_exit_status;
